...
class AutoDatabse extends database_1.Database {
    ...
    async autoCollection(collectionName, graphRef) {
        ...
        const vertexCollections = !graphRef
            ? await this.collections() : [];
        /* istanbul ignore next  */
        if (collectionNames.map((item) => item.name).indexOf(collectionName) === -1
            || vertexCollections.map((item) => item.name).indexOf(collectionName) === -1) {
            /* istanbul ignore next  */
            if (graphRef) {
