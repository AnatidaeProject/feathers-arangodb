...
const aql_query_1 = require("arangojs/lib/cjs/aql-query");
...
class DbService {
    ...
    async find(params) {
        ...
        const query = aql_query_1.aql.join([
            aql_query_1.aql `FOR doc in ${collection}`,
            queryBuilder.filter,
            queryBuilder.sort,
            queryBuilder.limit,
    ...
    async get(id, params) {
        ...
        const query = aql_query_1.aql.join([
            aql_query_1.aql `FOR doc IN ${collection}`,
            aql_query_1.aql `FILTER doc._key == ${id}`,
            queryBuilder.filter,
            queryBuilder.returnFilter
        ], " ");
    ...
    async _replaceOrPatch(fOpt = "REPLACE", id, data, params) {
        ...
        if (ids.length > 0 && (ids[0] != null || ids[0] != undefined)) {
            ...
            query = aql_query_1.aql.join([
                aql_query_1.aql `FOR doc IN ${ids}`,
                aql_query_1.aql.literal(`${fOpt}`),
                aql_query_1.aql `doc WITH ${data} IN ${collection}`,
                aql_query_1.aql `LET changed = NEW`,
                queryBuilder.returnFilter
            ], " ");
        }
        else {
    ...
    async remove(id, params) {
      ...
      `;
            ...
            query = aql_query_1.aql.join([
                aql_query_1.aql `FOR doc IN ${collection}`,
                queryBuilder.filter,
                aql_query_1.aql `REMOVE doc IN ${collection}`,
                aql_query_1.aql `LET removed = OLD`,
                queryBuilder.returnFilter
            ], " ");
        }
        return this._returnMap(database, query);
