...
defs.add('references', function (reference, values, query) {
  ...
  if (typeof reference === 'string') { return output + '"' + reference + '"'; }

  output += '"' + reference.table + '"';

  if (reference.column) { output += '("' + reference.column + '")'; }

  if (reference.onDelete) { output += ' on delete ' + reference.onDelete; }

  if (reference.onUpdate) { output += ' on update ' + reference.onUpdate; }

  if (reference.match) { output += ' match ' + reference.match; }
