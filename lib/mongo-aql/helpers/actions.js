...
actions.add('dropColumn', function (value, values, query) {
  ...
  if (value.ifExists) { output.push('if exists'); }

  output.push(utils.quoteObject(value.name));

  if (value.restrict) { output.push('restrict'); } else if (value.cascade) { output.push('cascade'); }
...
actions.add('alterColumn', function (value, values, query) {
  ...
  if (value.type) { output.push('type ' + value.type); }

  if (value.collation) { output.push('collate ' + value.collation); }

  if (value.using) { output.push('using (' + value.using + ')'); }

  if (value.default) { output.push('set default ' + value.default); }

  if (value.dropDefault) { output.push('drop default'); }

  if (value.notNull === true) { output.push('set not null'); }

  if (value.notNull === false) { output.push('drop not null'); }

  if (value.statistics) { output.push('set statistics $' + values.push(value.statistics)); }

  if (value.storage) { output.push('set storage ' + value.storage); }
