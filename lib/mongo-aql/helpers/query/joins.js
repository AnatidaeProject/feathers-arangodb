...
var buildJoin = function (join, values, query) {
  // Require a target
  if (!join.target) { throw new Error('Invalid join.target type `' + typeof join.target + '` for query helper `joins`'); }

  // Allow for strings or objects for join.on
  if (!join.on || (typeof join.on !== 'string' && typeof join.on !== 'object')) { throw new Error('Invalid join.on type `' + typeof join.on + '` for query helper `joins`'); }
