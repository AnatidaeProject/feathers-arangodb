...
helpers.register('limit', function (limit, values, query) {
  ...
  } else if (typeof limit === 'number') { return ' LIMIT ' + utils.newVar(limit, values); } else { throw new Error('Invalid limit type `' + typeof limit + '` for query helper `limit`. Limit must be number or \'all\''); }
});
