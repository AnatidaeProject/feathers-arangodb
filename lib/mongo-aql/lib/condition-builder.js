    ...
    for (var key in where) {
      ...
      if (typeof where[key] === 'object' && !(where[key] instanceof Date) && !Buffer.isBuffer(where[key])) {
        ...
        if (helpers.has(key)) {
          ...
          if (helpers.get(key).options.cascade) { (result = buildConditions(where[key], key, column, joiner, fields)) && conditions.push(result); } else { (result = helpers.get(key).fn(column, where[key], values, table, where[key])) && conditions.push(result); }
        }

        // Key is Joiner
        else if (key == '$or') { (result = buildConditions(where[key], condition, column, ' || ', fields)) && conditions.push(result); } else if (key == '$and') { (result = buildConditions(where[key], condition, column, ' && ', fields)) && conditions.push(result); }

        // Key is array index
        else if (+key >= 0) { (result = buildConditions(where[key], condition, column, joiner, fields)) && conditions.push(result); }
